// Portal.h — WiFi captive portal + NVS settings for INVERTYouTubeCompCYD
#pragma once
...
static void ptLoadSettings() {
    _pt_prefs.begin("ytcompcydi", true);
...
static void ptSaveSettings(const char* ssid, const char* wpass, const char* host, uint16_t port) {
    _pt_prefs.begin("ytcompcydi", false);
...
<h2>INVERT YouTubeCompCYD Setup</h2>
...
static void ptRunPortal() {
    WiFi.mode(WIFI_AP);
    WiFi.softAP("INVERT-YouTubeCompCYD");
