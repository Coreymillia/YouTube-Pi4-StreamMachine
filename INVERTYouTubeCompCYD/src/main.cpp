/**
 * INVERTYouTubeCompCYD — inverted-layout CYD dashboard for YouTubeCompanion.
 *
 * Uses the same companion API as YouTubeCompCYD but applies the touch
 * calibration pattern from the existing inverted CYD scanner project.
 */
...
static void mapTouch(uint16_t rx, uint16_t ry, int& sx, int& sy) {
    sx = constrain(map(rx, 200, 3900, 0, W), 0, W - 1);
    sy = constrain(map(ry, 240, 3900, 0, H), 0, H - 1);
}
